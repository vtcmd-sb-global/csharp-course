...
textCSharpSession1/                  ← Solution folder
│
...
ItemPurpose.slnSolution file. A solution can contain multiple projects..csprojProject file. Tells the compiler which .NET version to use, which packages to include, etc.Program.csThe file that contains the code that runs when you start the program.bin/ and obj/Automatically generated folders. You almost never edit these by hand.
...
FileResponsibilityProgram.csCreated the host and started the application (contained Main + CreateHostBuilder)Startup.csConfigured services (ConfigureServices) and the HTTP pipeline (Configure)
...
EraConsole AppASP.NET Core Web AppOlder (.NET 5 and earlier)Program.cs with explicit class + Main()Program.cs + Startup.csModern (.NET 6+)Program.cs with top-level statements (no visible Main)Single Program.cs (minimal hosting) – no Startup.cs by default
For this session we are only using console applications, so you will mainly work with the modern single-file Program.cs.
...
C#Console.WriteLine("Welcome to C#");
...
C#Console.Write("Enter your name: ");
string name = Console.ReadLine();
Console.WriteLine("Hello " + name);
Example:
textEnter your name: Ali
Hello Ali
...
Output:
textMy name is Ali and I am 20 years old.
